...
nit_NCO=N0*2+ N001*2   #NCO的数目
init_PTMG1000=N1*2      #OH的数目
init_PTMG2000=N2*2      #OH的数目
num_A=N0*28*3

#体系中所有粒子
all_particles=N001*2+ N0*92 +N1*14 +N2*28 +N3*82 +N4*2 +N5*2 +N6*25
print(init_NCO)
urethane_particles=['F1','L1','K011','K021','K031','C1','R1']
...
base_path = './'
for i in range(1000000,210000000, 1000000):  # 从 0 到 20100000
    ...
    if os.path.isfile(filepath):  # 检查文件是否存在
        ...
        if type_elem is not None:
            ...
            for line in lines:
                ...
                if type_value == 'F':# PTMG1000
                    numF += 1
                if type_value == 'R':# PPCD
                    numR += 1
        num_urethane=numF1+ numL1+ numK011+ numK021+ numK031+ numC1+ numR1
        # print(numE) 
        # print(numA) 
        # exit()   
        ...
        num_BDO.append(numC)
        num_PPCD.append(numR)
        file_indices.append(i / 10**6)  # 将文件索引除以 10^4，并添加到列表中
...
num_BDO=np.array(num_BDO) #BDO的羟基数目
num_PPCD=np.array(num_PPCD) #PTMG2000的羟基数目
# print(num_NCO)  
# exit()
...
# Plot num_BDO if not all zero
if not np.all(num_PPCD == 0):
    plt.plot(file_indices, num_PPCD, marker='^', linestyle='-', label='PPCD-OH Count')
